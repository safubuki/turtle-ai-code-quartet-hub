...
- 一括起動では A-D の対象スロットを 1 つずつ順番に起動し、最後に起動済みまたは再接続済みの管理対象ウィンドウを 2x2 配置する。非 CLI グループも並列起動しない。
- 起動後の即時補正と遅延補正は、期待位置から大きく外れている場合だけ座標を静かに戻す。追加補正ではレイヤー再適用やパネル前面化を繰り返さず、見た目のちらつきを抑える。
- VS Code は専用 `user-data-dir` 有効時は `code.lock` PID に一致するウィンドウを捕捉する。専用 user-data を使わない設定では、対象ワークスペース名がタイトルに出たウィンドウも補助的に採用する。起動が遅れた前スロットのウィンドウを次スロットとして誤割り当てしない。
- Workspace CLI はスロットごとに `cmd.exe` を開き、`Turtle {slot} - {shortName}` のタイトルで捕捉する。タイトルが反映されない場合は、新規 terminal ウィンドウを起動順で割り当てる。
