...
- Codex と GitHub Copilot Chat のログ候補は source 別に評価し、片方の実行中シグナルがもう片方の完了シグナルを隠さないようにします。
- Codex は明確な完了イベントを出さない場合があるため、ストリーム更新が止まったことだけを理由に完了とは推定しません。
- Copilot の実行中は、slot 別 user-data-dir の直近 `ccreq:` と UI Automation の明示状態を補助的に使います。`success` / `cancelled` / `networkError` / `markdown` は実行中シグナルから除外します。
